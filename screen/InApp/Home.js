...
export default function HomeScreen ({ navigation }) {
  React.useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerLeft: () => (
        // 뒤로가기 버튼 아이콘 설정
        <Text style={HomeStyle.backButton} onPress={() => navigation.goBack()}>
          {'〈'} {/* 뒤로가기 아이콘 */}
        </Text>
      ),
  ...
  const fetchDataFromFirebase = async () => {
    try {
      const snapshot = await database.ref('users/your_user_id/name').once('value');
